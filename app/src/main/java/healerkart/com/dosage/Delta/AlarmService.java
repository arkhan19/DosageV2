...
public class AlarmService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (matcher.matchAction(action)) {
			if (POPULATE.equals(action)) {
				DosageDB.dbHelper.populate(Long.parseLong(alarmId), DosageDB.db);
			...
			if (CANCEL.equals(action)) {
				...
				DosageDB.dbHelper.shred(DosageDB.db);
			}			
		}
	}
	...
	private void execute(String action, String... args) {		
		...
		if (alarmMsgId != null) {
			c = DosageDB.db.query(AlarmMsg.TABLE_NAME, null, AlarmMsg.COL_ID+" = ?", new String[]{alarmMsgId}, null, null, null);
			
		} else {
			c = AlarmMsg.list(DosageDB.db, alarmId, startTime, endTime, status);
		}
